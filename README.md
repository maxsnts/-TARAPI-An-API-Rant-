...
Or… even if File Systems? (sure i would not do it either!)
Homing Pigeons?
...
Why the hell do we need the status codes for then?
...
In here... the payload itself describes all of the operation instead of only part of it.
...
For the love of God don’t force your clients to keep record of your internal Ids!
...
Don’t you like to sleep well at night? So, instead of solving the problem on your side ONCE, you force each and EVERY client to solve the same problem over and over and over again.
...
So what, you identify the object by ClientID+ObjectID right? Where is the problem?
...
- ClientA send you a request where it asks the API to save ObjectID1 into you DB
...
Do it on the API!
Well, this is a deeper problem... I personaly thing that saving "BR" in you database on a piece of text that is not Html is just dumb! But i digress.
